...
class PixPro(nn.Module):
    def __init__(
        ...
        self.pixpro_pos_ratio       = pixpro_pos_ratio
    ...
    def forward_train(self, im_1, im_2, coord1, coord2):
        """
        Input:
        ...
        feat_1 = self.encoder(im_1)[0]  # queries: NxC
        feat_1 = self.encoder_avgpool(feat_1)
        proj_1 = self.projector(feat_1)
        pred_1 = self.featprop(proj_1)
        pred_1 = F.normalize(pred_1, dim=1)

        feat_2 = self.encoder(im_2)[0]
        feat_2 = self.encoder_avgpool(feat_2)
